...
class DataLabelManager {
    ...
    calculateTransform(viewport, addTransform) {
        if (addTransform) {
            return {
                x: viewport.width / 2,
                y: viewport.height / 2
    ...
    processDataPoints(data, layout, transform, arrangeGrid, viewport, hideCollidedLabels) {
        const filteredData = [];
    ...
    shouldKeepLabel(position, arrangeGrid, info, viewport, hideCollidedLabels) {
    ...
    */
    ...
    calculateContentPosition(anchoredElementInfo, contentSize, offset) {
        ...
        if (isRectBasedPosition) {
            return this.calculateContentPositionFromRect(anchorRect, anchorRectOrientation, contentPosition, contentSize, offset);
        }
        return this.calculateContentPositionFromPoint(anchorPoint, contentPosition, contentSize, offset);
    }
    ...
    createViewportIntersection(position, size) {
    ...
    checkOutsidePlacementCollisions(placement, intersection, position) {
        switch (placement) {
...
export default DataLabelManager;
